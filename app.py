...
def render_news_ticker():
    ...
    txt = " \u2022 ".join(news)
    st.markdown(
        "<style>@keyframes mg_news{from{transform:translateX(0)}"
        ...
        f'<div style="display:inline-block;white-space:nowrap;color:#eaf2ff;font-weight:600;'
        f'font-size:.82rem;animation:mg_news 60s linear infinite">'
        f'\u26a1 {txt}&nbsp;&nbsp;&nbsp;\u26a1 {txt}</div></div>',
        unsafe_allow_html=True)
...
if render_market_pulse is not None:
    try:
        render_market_pulse()
    except Exception as _e:
        st.caption(f"Marknadspuls kunde inte laddas: {_e}")
